...
sub submit
{
    ...
    my $vendor = $description->vendor();
    if (defined ($vendor)) {
        $self->log("  User requested vendor " . $vendor . "\n");
        $rc = $sge_job_script->print("#\$ -l vendor=". $vendor ."\n");
        if (!$rc)
        {
            return $self->respond_with_failure_extension(
                    "print: $sge_job_script_name: $!",
    ...
    my $memory = $description->memory();
    if (defined ($memory)) {
        $self->log("  User requested memory " . $memory . "\n");
        $rc = $sge_job_script->print("#\$ -l memory=". $memory ."\n");
        if (!$rc)
        {
            return $self->respond_with_failure_extension(
                    "print: $sge_job_script_name: $!",
    ...
    my $scratch = $description->scratch();
    if (defined ($scratch)) {
        $self->log("  User requested scratch " . $scratch . "\n");
        $rc = $sge_job_script->print("#\$ -l scratch=". $scratch ."\n");
        if (!$rc)
        {
            return $self->respond_with_failure_extension(
                    "print: $sge_job_script_name: $!",
